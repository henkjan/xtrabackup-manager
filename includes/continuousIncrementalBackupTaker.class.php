		...
		function takeScheduledBackupSnapshot ( $scheduledBackup = false ) {
			...
			while($readyToRun == false ) {
				...
				if($scheduledBackup->isRunning()) {
					...
					throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Detected this scheduled backup job is already running.");
				}
					...
					***********************/
					...
					try {
						...
						if( !is_resource($xbProc) ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to use ssh to start xtrabackup with: $xbCommand .");
						}   
						...
						do {
							...
							if( ! ( $xbStatus = proc_get_status($xbProc) ) ) {
								throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to retrieve status on backup process.");
							}
							sleep(5);
						...
						if($xbStatus['exitcode'] <> 0 ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: There was an error backing up - The process returned code ".$xbStatus['exitcode'].".".
						...
						if(!is_resource($ncProc) ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to start netcat with command: $ncCommand .");
						}
						...
						if(strlen($ncOutput) > 0 ) {
							...
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to start netcat with command: $ncCommand . Got error output: $ncOutput .");
						}
						...
						if( !is_resource($copyProc) ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to use ssh to start copy with: $copyCommand .");
						}
						...
						do {
							...
							if( ! ( $copyStatus = proc_get_status($copyProc) ) ) {
								throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to retrieve status on copy process.");
							}
							sleep(5);
						...
						if($copyStatus['exitcode'] <> 0 ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: There was an error copying files - The process returned code ".$copyStatus['exitcode'].".".
													" The output from the backup is as follows:\n".$streamContents);
						}
					...
					$this->applyRetentionPolicy($scheduledBackup);
				...
				// If there is not a valid seed, take a full backup
					...
					***********************/
					...
					try {
						...
						// Check that we launched OK.
						if( !is_resource($xbProc) ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to use ssh to start innobackupex with: $xbCommand .");
						}
...
/*						print_r($xbStatus);
						...
						if( !is_resource($applyProc) ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to run apply log command: $applyCommand");
						}
						...
						do {
							...
							if( ! ( $applyStatus = proc_get_status($applyProc) ) ) {
								throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: Unable to retrieve status on apply log process.");
							}
							sleep(5);
						...
						if($applyStatus['exitcode'] <> 0 ) {
							throw new Exception('continuousIncrementalBackupTaker->takeScheduledBackupSnapshot: '."Error: There was an error applying logs - The process returned code ".$applyStatus['exitcode'].".".
